...
- (id) initWithParent: (CCNode*)parent_ text: (NSString*)text otherside:(BOOL)left
{
	if( (self = [super init])) {
		...
		self.position = ccp(xNum, -400);
		
		parent = parent_;
		[parent addChild:self];
		
		[self runAction: [CCMoveTo actionWithDuration:0.5 position:ccp(xNum, -120)]];
		
		id removeAction = [CCCallBlock actionWithBlock:^{
			[parent removeChild:self cleanup:true];
		}];
		
		[self runAction: [CCSequence actions:[CCDelayTime actionWithDuration:4], [CCMoveTo actionWithDuration:1 position:ccp(xNum, -400)], removeAction, nil] ];
	}
	return self;
}
